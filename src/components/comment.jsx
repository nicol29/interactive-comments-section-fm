  ...
  return (
    <article key={uniqid()} className={"comment"}>
      ...
      <div className="comment-interactables">
        <div className="vote-buttons" data-id={comment.id}>
          <button onClick={() => dispatch({ type: "upvote", id: comment.id})}>+</button>
          <p>{comment.score}</p>
          <button onClick={() => dispatch({ type: "downvote", id: comment.id})}>-</button>
        </div>
...
// onClick={() => dispatch({ type: "delete", id: comment.id, nested: nested})}
